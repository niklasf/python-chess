...
class ZobristHasher(object):
    ...
    Class variables:
        POLYGLOT_RANDOM_ARRAY: The random array as used by Polyglot opening
            books.
    """
    ...
    def __init__(self, random_array=None):
        ...
        Args:
            random_array: An tuple of 781 pseudo-random 64 bit integers.
    ...
    def hash_position(self, position):
        ...
        if position.get_castling_right("K"):
            key ^= self._random_array[768]
        if position.get_castling_right("Q"):
            key ^= self._random_array[768 + 1]
        if position.get_castling_right("k"):
            key ^= self._random_array[768 + 2]
        if position.get_castling_right("q"):
